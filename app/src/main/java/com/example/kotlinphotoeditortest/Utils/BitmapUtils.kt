...
object BitmapUtils {
    ...
    private fun storeThumbnail(contentResolver: ContentResolver, source: Bitmap?, id :Long ,height: Float, width: Float, micrO_KIND: Int) :Bitmap? {
    ...
    private fun calculateInSampleSize(options: BitmapFactory.Options, regWidth: Int, regHeight: Int): Int {
        ...
        if (height > regHeight || width > regWidth){
            ...
            while (halfHeight / inSimpleSize >= regHeight && halfWidht / inSimpleSize >= regHeight)
            inSimpleSize *=2
        }
